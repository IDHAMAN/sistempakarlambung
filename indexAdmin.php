...
$jumlahPasien = mysqli_query($koneksi, "SELECT COUNT('username') as jml_pasien FROM user WHERE role='1'");
    ...
    <div class="card-body">
        <form method="post" encytpe="multipart/form-data">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                    <a class="badge badge-pill badge-primary" href="ubahPasien.php?id_user=<?php echo $data["username"]; ?>">edit</a> |
                    <a href="function.php?act=hapusPasien&username=<?= $data["username"]; ?>" onclick="return confirm('Yakin ingin menghapus data?');" class="badge badge-pill badge-danger">hapus</a>
                    </td>
                    <td><?= $data['username']; ?></td>
                    <td><?= $data['nama']; ?></td>
                    <td><?= $data['email']; ?></td>
